...
app.get ($.index, async function (app: any, request: any, response: any, next: any) {
	// var {results} = await request.database.prepare ("SELECT * FROM customer").run ()
	var results = await request.database.prepare ("insert into customer (name, contact) values (?, ?)").bind ("Bacot", "Rahmat").run ()
	console.log (results)
...
app.get ("/tests", async function (app: any, request: any, response: any, next: any) {
