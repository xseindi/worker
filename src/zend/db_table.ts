var table: any = {
	...
	"visitor:session": {name: "table_visitor_session", sql: `create table if not exists (id integer primary key, reference integer, date text, ip text, country text, agent text, description text);`},
	"file:transfer-queue": {name: "table_file_transfer_queue", sql: `create table if not exists (id integer primary key, reference integer, tmdb integer, imdb text, description text);`},
