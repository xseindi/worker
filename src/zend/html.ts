...
php.html = function (output: string, variable: any = {}) {
	...
	if (variable ["ad adsterra:float"]) markup.push (2, `<script type="text/javascript" src="${variable ["ad adsterra:float"]}"></script>`);
	markup.push (1, `</body>`);
	markup.push (0, `</html>`);
...
php.help.scriptag = function (app: any, request: any, response: any, next: any) {
	...
	markup.push (2, `<script>vue.app.reference = ${request.client.reference}</script>`);
	...
	markup.push (2, `<script>vue.app.variable = ${JSON.stringify (response.app.variable)}</script>`);
	markup.push (2, `<script>vue.app.config ["AD__.s"] = ${response.app.config ["AD__.s"]}</script>`);
...
php.help ["ld+json"].website = function () {
	return JSON.stringify ({
		...
		"potentialAction": {
			"@type": "SearchAction",
			"target": {"@type": "EntryPoint", "urlTemplate": "{{ base_url }}{{ router search }}?query={search_term_string}"},
...
php.page ["privacy-policy"] = function () {
	return `<p>Last updated: {{ var:last_update }}</p>
						...
						<li>
						<p><strong>Company</strong> (referred to as either &quot;the Company&quot;, &quot;We&quot;, &quot;Us&quot; or &quot;Our&quot; in this Agreement) refers to {{ var:name }}.</p>
						</li>
						<li>
						...
						<li>
						<p><strong>Website</strong> refers to {{ var:name }}, accessible from <a href="{{ var:base_url }}" rel="external nofollow noopener" target="_blank">{{ var:base_url }}</a></p>
						</li>
						<li>
						...
						<ul>
						<li>
						<p>By email: {{ var:email }}</p>
						</li>
						<li>
						<p>By visiting this page on our website: <a href="{{ var:url }}" rel="external nofollow noopener" target="_blank">{{ var:url }}</a></p>
						</li>
						</ul>`;
	}
