...
var TABLE: any = {
	...
	"client": {name: "table_client", sql: `create table if not exists table_client (id integer primary key, reference integer, host text, theme_id text, theme_type text, theme_version text, json text not null default '{}');`},
	"blob": {name: "table_blob", sql: `create table if not exists table_blob (id integer primary key, reference integer, key text, value blob);`},
	"image": {name: "table_image", sql: `create table if not exists table_image (id integer primary key, reference integer, key text, value text);`},
